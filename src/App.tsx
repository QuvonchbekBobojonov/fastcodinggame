...
const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
  [
    ...
    isActive ? 'bg-[#0A4A8A] text-white' : 'text-[#0f172a] hover:bg-white/60',
  ].join(' ')
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10">
        <header className="flex flex-wrap items-center justify-between gap-4 rounded-3xl border border-[#e5e5e5] bg-white px-6 py-4 shadow-[0_20px_50px_rgba(15,23,42,0.06)]">
          ...
          <nav className="flex flex-wrap items-center gap-2">
            ...
            <label className="flex items-center gap-2 rounded-full border border-[#e5e5e5] bg-[#f9fafb] px-3 py-1 text-xs font-semibold text-[#0f172a]">
              <span>{t('language.selectLabel')}</span>
              <select
                aria-label={t('language.selectLabel')}
                className="bg-transparent text-sm font-bold text-[#0f172a] outline-none"
                value={locale}
